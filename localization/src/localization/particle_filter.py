...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        theta_mean = utils.quaternion_to_angle(msg.orientation)

        particles[:, 0] = np.random.normal(x_mean, self.x_std, n_particles)
        particles[:, 1] = np.random.normal(y_mean, self.y_std, n_particles)
        particles[:, 2] = np.random.normal(theta_mean, self.theta_std, n_particles)
