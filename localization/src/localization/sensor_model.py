...
class SingleBeamSensorModel:
    ...
    def precompute_sensor_model(self, max_r):
            ...
            # p_hit (Gaussian distribution around the expected measurement)
            p_hit = np.zeros_like(obs_r, dtype=float)
            if self.z_hit > 0:
                p_hit = np.exp(-0.5 * diff**2 / self.hit_std**2) / (self.hit_std * np.sqrt(2*np.pi))
                p_hit[(obs_r > max_r) | (obs_r < 0)] = 0.0

            # p_short (linear probability)
            p_short = np.zeros_like(obs_r, dtype=float)
            if self.z_short > 0:
                cond = (obs_r >= 0) & (obs_r < sim_r - 1e-6) & (obs_r >=0) & (sim_r > 1e-6)
                p_short[cond] = 2 * (sim_r[cond] - obs_r[cond]) / sim_r[cond]**2

            # p_max (max ranges)
            p_max = np.zeros_like(obs_r, dtype=float)
            if self.z_max > 0:
                p_max[(np.abs(obs_r - max_r) < 1e-6)] = 1.0

            # p_rand (random noise across the entire range)
            p_rand = np.zeros_like(obs_r, dtype=float)
            if self.z_rand > 0:
                if max_r > 0:
                    p_rand[(obs_r >= 0) & (obs_r < max_r - 1e-6)] = 1.0 / max_r
            
            prob_table = (self.z_hit * p_hit) + (self.z_short * p_short) + (self.z_max * p_max) + (self.z_rand * p_rand)
            prob_table[np.isnan(prob_table) | np.isinf(prob_table)] = 0.0

            column_sums = prob_table.sum(axis=0, keepdims=True)
            cond = column_sums == 0
            column_sums[cond] = 1.0
            prob_table /= column_sums
            prob_table[:, cond[0]] = np.nan
