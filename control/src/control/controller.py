...
def compute_position_in_frame(p, frame):
    ...
    e_x = np.cos(ftheta) * (px - fx) + np.sin(ftheta) * (py - fy)
    e_y = -np.sin(ftheta) * (px - fx) + np.cos(ftheta) * (py - fy)
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            current_state = np.array(pose[:2])
            path_pos = path_xytv[:, :2]

            distances = np.linalg.norm(path_pos - current_state, axis=1)
            min_distances_idx = np.argmin(distances)
            lookahead_distances = distances[min_distances_idx:] - distance_lookahead
            positive_distances = np.where(lookahead_distances >= 0)[0]

            if positive_distances.any():
                idx = min_distances_idx + positive_distances[0]
                # if idx > min_distances_idx and abs(distances[idx] - distance_lookahead) > abs(distances[idx-1] - distance_lookahead):
                #     return idx - 1
                return idx
