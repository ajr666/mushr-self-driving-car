...
class ModelPredictiveController(BaseController):
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for t in range(self.T):
            state_changes = self.motion_model.compute_changes(rollouts[:, t, :], controls[:, t, :], dt)
            rollouts[:, t + 1, :] = rollouts[:, t, :] + state_changes
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        # Extract final state position (x, y) of each rollout
        final_positions = rollouts[:, -1, :2]
        distances = np.linalg.norm(final_positions - reference_xyt[:2], axis=1)
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # Reshape rollouts to (K * (T+1), 3) for collision checking
        reshaped_rollouts = rollouts.reshape(-1, 3)

        # Check collisions in map for all states in the rollouts
        collision_results = self.check_collisions_in_map(reshaped_rollouts)

        # Reshape collision results back to (K, T+1)
        collision_results = collision_results.reshape(self.K, self.T + 1)

        # Sum collisions for each rollout and apply weight
        costs = np.sum(collision_results, axis=1) * self.collision_w
        return costs
        # END QUESTION 4.3
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        # BEGIN QUESTION 4.4
        # Generate rollouts for each control sequence
        rollouts = self.get_rollout(pose, self.sampled_controls)
        # END QUESTION 4.4
        
        # BEGIN QUESTION 4.4
        
        distance_costs = self.compute_distance_cost(rollouts, reference_xytv[:3])
        collision_costs = self.compute_collision_cost(rollouts, None)
        costs = distance_costs + collision_costs
        ...
        return self.sampled_controls[best_index, 0]
        # END QUESTION 4.4
